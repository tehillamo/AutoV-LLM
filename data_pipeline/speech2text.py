...
def create_csv(path, output_dir, model):
    """
    ...
    Parameters:
        ...
        output_dir (str): The path to the directory where the CSV file will be saved.
    ...
    with open(os.path.join(output_dir, 'transcribed.csv'), 'w', newline='') as csvfile:
        ...
        for uuid in uuids:
            ...
            transcribed = transcribe_audios(paths, model)
...
def transcribe_audios(paths, model):
    """
...
def main():
    ...
    parser.add_argument("-i", "--input-path", dest="path", required=True, help="Path of ressource folder containing the folders with uuids")
    ...
    parser.add_argument("-m", "--model", default="base", dest="model", help="Model to use for transcribing (openai-whisper). Available models: (english only): tiny.en, base.en, small.en, medium.en (multilingual): tiny, base, small, medium, large. For more information see https://github.com/openai/whisper")
    ...
    if args.transcribe is True:
        ...
        create_csv(args.path, args.output_path, args.model)
